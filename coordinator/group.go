...
func AttachRSToGroup_local(rs RemoteServer) RegisterReply {
  ...
  if rs.Coordinator {
    return RegisterReply{remoteCoordinators,remoteDaemons,failures,shards,rs.ID}
  }else {
    return RegisterReply{nil,nil,0,0,rs.ID}
  }
...
func AttachToGroup(groupAddress string, groupPort string) {
  ...
  verboseLog("tolerate failues:",failures)
  verboseLog("shards:",shards)
